...
class Epsilon_Greedy_Exploration(Base_Exploration_Strategy):
    ...
    def __init__(self, config):
        ...
        self.writer = SummaryWriter(comment='_'+config.env_title+TIME)
        ...
        ## détermine combien d'épisodes sont exécutés pour les actions de sélection aléatoire  default = 0
    ...
    def get_updated_epsilon_exploration(self, action_info, epsilon=1.0, force_explore=False):
        ...
        if self.exploration_cycle_episodes_length is None:
            ...
            self.writer.add_scalar("Epsilon",epsilon, episode_number)
            # epsilon = 0.1
        else:
